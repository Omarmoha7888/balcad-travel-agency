...
import { X, Globe, PhoneCall, MessageSquare, ShieldAlert, ChevronRight, ClipboardList } from 'lucide-react';
import { Logo } from './Logo';

interface SidebarMenuProps {
  isOpen: boolean;
  onClose: () => void;
  activeTab: 'products' | 'about-us' | 'live-chat' | 'admin' | 'my-orders';
  setActiveTab: (tab: 'products' | 'about-us' | 'live-chat' | 'admin' | 'my-orders') => void;
}
...
export default function SidebarMenu({
  ...
  const handleLinkClick = (tabId: 'products' | 'about-us' | 'live-chat' | 'admin' | 'my-orders') => {
    onClose();
    setActiveTab(tabId);
  };
            ...
            <div>
              <div className="p-4 border-b border-zinc-100 dark:border-zinc-800 flex items-center justify-between bg-gradient-to-r from-amber-400 to-yellow-400 text-zinc-950">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h2 className="font-bold text-sm tracking-tight text-zinc-950">Balcad Travel</h2>
              ...
              <div className="p-4 space-y-2">
                <p className="text-[11px] font-bold text-zinc-450 dark:text-zinc-550 uppercase tracking-wider px-2">
                  Qaybaha Shirkadda
                </p>
                ...
                <button
                  ...
                  className={`w-full flex items-center justify-between p-3 rounded-xl text-left transition-all cursor-pointer ${
                    activeTab === 'products'
                      ? 'bg-amber-50 dark:bg-amber-955/25 text-amber-700 dark:text-amber-400 font-semibold border border-amber-400/20'
                ...
                <button
                  ...
                  className={`w-full flex items-center justify-between p-3 rounded-xl text-left transition-all cursor-pointer ${
                    activeTab === 'about-us'
                      ? 'bg-emerald-50 dark:bg-emerald-955/20 text-emerald-700 dark:text-emerald-400 font-semibold border border-emerald-500/15'
                      : 'hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-700 dark:text-zinc-300'
                  }`}
                >
                  <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className={`w-full flex items-center justify-between p-3 rounded-xl text-left transition-all cursor-pointer ${
                    activeTab === 'live-chat'
                      ? 'bg-blue-50 dark:bg-blue-955/20 text-blue-700 dark:text-blue-400 font-semibold border border-blue-500/15'
                      : 'hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-700 dark:text-zinc-300'
                  }`}
                >
                  <div className="flex items-center gap-3">
                ...
                {/* Section 4 Link (Dalabyada / My Orders) */}
                <button
                  type="button"
                  onClick={() => handleLinkClick('my-orders')}
                  className={`w-full flex items-center justify-between p-3 rounded-xl text-left transition-all cursor-pointer ${
                    activeTab === 'my-orders'
                      ? 'bg-amber-50 dark:bg-amber-955/20 text-amber-700 dark:text-amber-400 font-semibold border border-amber-550/15'
                      : 'hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-700 dark:text-zinc-300'
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <ClipboardList className="w-4 h-4 text-amber-550 text-amber-500" />
                    <span>Dalabyada (My Orders)</span>
                  </div>
                  <ChevronRight className="w-4 h-4 opacity-50" />
                </button>
